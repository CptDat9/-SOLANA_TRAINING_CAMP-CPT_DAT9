 ...
 */
// export const payer = process.env?.LOCAL_PAYER_JSON_ABSPATHz
  // ? loadKeypairFromFile(process.env?.LOCAL_PAYER_JSON_ABSPATH)
  // : loadOrGenerateKeypair("payer");
import { Keypair } from "@solana/web3.js";

export const payer = process.env.PAYER_SECRET_KEY
  ? Keypair.fromSecretKey(Buffer.from(JSON.parse(process.env.PAYER_SECRET_KEY)))
  : loadOrGenerateKeypair("payer");
